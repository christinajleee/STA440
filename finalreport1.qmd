...
Therefore, the beta-binomial model gives us more realistic confidence intervals around the estimated pass rates and enables us to model the inherent heterogeneity among cohorts. The logistic regression model was not a good fit because even though the p-values were significant, the residual deviance was 539.15, which is greater than the expected value (17). Additionally, the AIC of the model was 714.13, which is greater than the AIC results of the Beta Binomial model.
...
kable(
  head(data),
  caption = "Table 1. First six rows of the internal medicine exam dataset (N = # of exam takers, Pct = % passed)."
)
```
...
kable(
  period_means,
  caption = "Table 2. Average pass rate by reform period (pre-2003, 2003–2010, and 2011+)"
)
```
...
summary_table |>
  gt() |>
  tab_header(
    title = "Table 3. Beta-binomial regression estimates by reform period"
  ) |>
  tab_options(
...
deviance_resid <- residuals(binomial_model, type = "deviance")

pearson_chisq <- sum(pearson_resid^2)
df_res <- binomial_model$df.residual
pearson_p_value <- pchisq(pearson_chisq, df_res, lower.tail = FALSE)

cat("=== Pearson Residuals ===\n")
cat("Pearson Chi-square:", pearson_chisq, "\n")
cat("Residual df:", df_res, "\n")
cat("Goodness-of-fit p-value:", pearson_p_value, "\n\n")

cat("=== Deviance Residuals ===\n")
cat("Residual Deviance:", binomial_model$deviance, "\n")
cat("Residual df:", df_res, "\n")
cat("Lack-of-fit p-value:",
    pchisq(binomial_model$deviance, df_res, lower.tail = FALSE), "\n")
...
kable(
  aic_table,
  booktabs = TRUE,
  caption = "AIC comparison of Binomial vs Beta-binomial models"
)

kable(
  data.frame(Dispersion_Coef_Logit = round(disp_coef, 3),
             Rho = round(disp_param, 4)),
  booktabs = TRUE,
  caption = "Dispersion coefficient (logit scale) and estimated dispersion parameter (ρ)."
)

colnames(pred_table) <- c("Period", "Predicted Prob Mu", "Predicted Prob Rho","CI Lower Mu", "CI Lower Rho", "CI Upper Mu", "CI Upper Rho")

kable(
  pred_table,
  booktabs = TRUE,
  caption = "Table 3. Predicted pass probabilities by reform period (95% CI)."
)
```
