...
```
```{r}
# Add centered year (to capture trends within each period)
data$Year_c <- data$Year - 2003

# Logistic regression with period + linear year trend interaction
trend_mod <- glm(cbind(Pass, Fail) ~ Period * Year_c,
                 family = binomial, data = data)
