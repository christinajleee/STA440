...
odds_ratios <- c(exp(coef_all[1:3]), NA)
pred_probs <- c(exp(coef_all[1:3])/(1+exp(coef_all[1:3])), NA)

ci_lower <- c(exp(coef_all[1:3] - 1.96*se_all[1:3]), NA)
ci_upper <- c(exp(coef_all[1:3] + 1.96*se_all[1:3]), NA)

summary_table <- data.frame(
  Predictor = c("(Intercept) mu", "(Intercept) rho", "timeperiodtp2", "timeperiodtp3"),
  Log_Odds = round(coef_all[c(1,4,2,3)],3),
  Std_Error = round(se_all[c(1,4,2,3)],3),
  Z_value = round(z_all[c(1,4,2,3)],3),
  P_value = signif(p_all[c(1,4,2,3)],3),
  Odds_Ratio = round(odds_ratios[c(1,4,2,3)],3),
  CI_lower = round(ci_lower[c(1,4,2,3)],3),
  CI_upper = round(ci_upper[c(1,4,2,3)],3),
  Predicted_Prob = round(pred_probs[c(1,4,2,3)],3)
)

print("=== Model Summary Table (including rho) ===")
print(summary_table)
...
```{r}
ggplot(x, aes(x = Year)) +
  geom_point(aes(y = Pct), color = "blue", size = 2, alpha = 0.7) +
  ...
  labs(title = "Medical Residents Exam Pass Rates",
